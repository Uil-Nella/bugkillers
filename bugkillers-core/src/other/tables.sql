...
CREATE TABLE `bk_user` (
  ...
  `bk_create` datetime NOT NULL DEFAULT now(),
  `bk_modified` datetime NOT NULL DEFAULT now(),
...
CREATE TABLE `bk_question` (
  ...
  `question_status` int(4) NOT NULL COMMENT '问题状态 1 待回答 2 已经回答 3 已解决 ',
  `question_times` int(10) DEFAULT 0 COMMENT '浏览次数 每被查询一次加一',
  `bk_create` datetime NOT NULL DEFAULT now(),
  `bk_modified` datetime NOT NULL DEFAULT now(),
  `dr` int(4) unsigned NOT NULL DEFAULT '1' COMMENT '1=正常  2=删除',
  PRIMARY KEY (`id`)
...
CREATE TABLE `bk_prestige` (
  ...
  `presige_value` int(10) NOT NULL COMMENT '声望值',
  `zan_value` int(10) NOT NULL COMMENT '被赞次数',
  `bk_create` datetime NOT NULL DEFAULT now(),
  `bk_modified` datetime NOT NULL DEFAULT now(),
  `dr` int(4) unsigned NOT NULL DEFAULT '1' COMMENT '1=正常  2=删除',
  PRIMARY KEY (`id`)
